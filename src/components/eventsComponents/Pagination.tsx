...
import { maxFollowPageAtom } from "../../pages/Events";

export const currentFollowersPageAtom = atom({
    key: "CurrentFollowersPage",
    default: 1
})

interface FollowersListPagProps {
    disabled: boolean
}

const MainListPagination = ({ disabled }: FollowersListPagProps) => {
    const [currentPage, setCurrentPage] = useRecoilState(currentFollowersPageAtom);
    const allPage = useRecoilValue(maxFollowPageAtom);
